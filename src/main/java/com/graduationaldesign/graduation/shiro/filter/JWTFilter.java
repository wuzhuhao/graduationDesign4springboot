package com.graduationaldesign.graduation.shiro.filter;
...
import com.graduationaldesign.graduation.commons.CommonConstant;
...
//    private IUserService userService;
    ...
    protected boolean onAccessDenied(ServletRequest servletRequest, ServletResponse servletResponse)
        ...
        if (token.getPrincipal() == null) {
            handler401(servletResponse, CodeAndMsgEnum.UNAUTHENTIC.getcode(),
                    CodeAndMsgEnum.UNAUTHENTIC.getMsg());
            return false;
        } else {
            try {
                //放行，进入shiro部分
